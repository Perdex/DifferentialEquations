...
public class Particle2D {
    ...
    static void interact(double dt, double dx, double dy, Particle2D[][] ps,
        double[][] mask, int fromi, int toi, int fromj, int toj){
            ...
            //            avgy = (p[idx][idy - 1].u + p[idx][idy].u) / 2;
                ...
                switch(p.type){
                    case wave:
                        ...
                        // is integrated to
                        // ut += dt * ...
                        p.ut += dt * (p.type.coeffs[0] * mask[i][j] * uxx - p.type.coeffs[1] * p.ut);
                        p.ut += dt * (p.type.coeffs[0] * mask[i][j] * uyy - p.type.coeffs[1] * p.ut);
                        break;
